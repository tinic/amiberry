...
/* Repair offload-partial transport checksums; see ethernet_csum.h for what
   that means and why the pcap backend is where it has to happen.
   AMIBERRY_NET_CSUM_FIX=0 disables.
...
void ethernet_fix_partial_csum(uae_u8 *d, int len)
{
	...
	if (!on)
		return;
	if (ethernet_csum_fix_frame(d, len))
		ethernet_csum_fixed++;
}
